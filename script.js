...
// new table code for gender sort starts;
let genf=document.createElement("table");
genf.innerHTML=`
<thead><tr>
    <th>ID</th>
    <th>Name</th>
...
genf.style.cssText=`border: 1px solid black;
...
font-weight: 400;
width: 95%;`;
document.getElementById("table").appendChild(genf);
// new table for gender code ends
function render(){
    genf.remove();
    array.forEach((item,i)=>{
content.innerHTML+=`<tr><td>${item.id}</td>
<td><img width='20px' height='20px' style='border:1px solid black; border-radius:50%;' src="${item.img_src}"> ${item.first_name} ${item.last_name}</td>
<td>${item.gender}</td>
<td>${item.class}</td>
<td>${item.marks}</td>
<td id='pass${i}'></td>
<td>${item.email}</td></tr>`;
let temp=document.getElementById(`pass${i}`);
if (item.passing) {
    temp.textContent=`Passed`;
}
else{
    temp.textContent="Failed";
}
});
}
render();
...
let arr1=[];
array.forEach((item,i)=>{
arr1.push(`${item.first_name+item.last_name}`);
});
arr1.sort();
sortaz.addEventListener("click",()=>{ 
    ...
    content.innerHTML=``;
        arr1.forEach((elem,j)=>{
            array.forEach((item,i)=>{
            if(elem==item.first_name+item.last_name){
                content.innerHTML+=`<tr><td>${item.id}</td>
                <td><img width='20px' height='20px' style='border:1px solid black; border-radius:50%;' src="${item.img_src}"> ${item.first_name} ${item.last_name}</td>
                <td>${item.gender}</td>
                <td>${item.class}</td>
...
let marks=[];
array.forEach((item,i)=>{
    marks.push(parseInt(item.marks));
    });
    marks.sort((a,b)=>{
        return a-b;
    });
...
// sort by passed
    let sortp=document.getElementById("sortP");
    sortp.addEventListener("click",()=>{
        genf.remove();
        content.innerHTML=``;
        array.forEach((item,i)=>{
if (item.passing) {content.innerHTML+=`<tr><td>${item.id}</td>
    <td><img width='20px' height='20px' style='border:1px solid black; border-radius:50%;' src="${item.img_src}"> ${item.first_name} ${item.last_name}</td>
    <td>${item.gender}</td>
    <td>${item.class}</td>
    <td>${item.marks}</td>
...
// sort by class
let sortc=document.getElementById("sortC");
let clas=[];
array.forEach((item,i)=>{
    clas.push(parseInt(item.class));
    });
    clas.sort((a,b)=>{
        return a-b;
    });
...
sortc.addEventListener("click",()=>{
    genf.remove();
    content.innerHTML=``;
nclass.forEach((elem,j)=>{
    array.forEach((item,i)=>{
    if(elem==item.class){
        content.innerHTML+=`<tr><td>${item.id}</td>
        <td><img width='20px' height='20px' style='border:1px solid black; border-radius:50%;' src="${item.img_src}"> ${item.first_name} ${item.last_name}</td>
...
//sort by gender
let sortg=document.getElementById("sortG");
sortg.addEventListener("click",()=>{
    content.innerHTML=``;
    array.forEach((item,i)=>{
if (item.gender=="Male") {content.innerHTML+=`<tr><td>${item.id}</td>
<td><img width='20px' height='20px' style='border:1px solid black; border-radius:50%;' src="${item.img_src}"> ${item.first_name} ${item.last_name}</td>
<td>${item.gender}</td>
<td>${item.class}</td>
<td>${item.marks}</td>
...
let temp=document.getElementById(`pass${i}`);
   ...
   // new table some code above
   document.getElementById("table").appendChild(genf);
let content1=document.getElementById("hello");
content1.innerHTML=``;
   array.forEach((item,i)=>{
    if (item.gender=="Female") {
        content1.innerHTML+=`<tr><td>${item.id}</td>
    <td><img width='20px' height='20px' style='border:1px solid black; border-radius:50%;' src="${item.img_src}"> ${item.first_name} ${item.last_name}</td>
    <td>${item.gender}</td>
    <td>${item.class}</td>
    <td>${item.marks}</td>
